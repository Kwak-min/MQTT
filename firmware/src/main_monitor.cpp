 ...
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 *
 * [DEPRECATED — 2026-06] firmware/src/main_monitor.cpp
 * ═══════════════════════════════════════════════════════════════════════════
 * 이 파일은 더 이상 빌드 대상이 아닙니다.
 *
 * 폐기 이유:
 *   Board 3 (ESP32-S3 + INA219 이중 채널)을 이용한 하드웨어 기반 전력 계측이
 *   IEEE Access 2024 (DOI: 10.1109/ACCESS.2024.3523864) 기반의 소프트웨어
 *   정의 전력 추정 모델로 완전히 대체되었습니다.
 *
 * 대체 구현 위치:
 *   backend/app/services/power_estimator.py
 *   — RTT, retry_count, QoS, sleep_mode_ratio를 기반으로 소비 전력을 추정합니다.
 *   — 별도 하드웨어(Board 3) 없이 게이트웨이(Raspberry Pi 5)에서 실행됩니다.
 *
 * 보존 이유:
 *   하드웨어 계측 접근법의 참조 구현 및 실험 재현성을 위해 소스를 보존합니다.
 *   완전히 삭제하려면 이 파일을 수동으로 제거하세요.
 *
 * 빌드 제외 방법 (platformio.ini):
 *   [env:board3_monitor]
 *   build_src_filter = -<*>   ; 아무것도 빌드하지 않음
 * ═══════════════════════════════════════════════════════════════════════════
 */
