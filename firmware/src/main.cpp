 ...
 * 역할           : 엣지 AI 센서 노드 — 환경 데이터 수집 → 동적 QoS 선택 → UDP 전송
 *
 * ┌─────────────────────────────────────────────────────────────────────────┐
 ...
 * │    - preprocess_tinyml_inputs() 내 POWER_MODE 분기:                     │
 * │        EXTERNAL_5V → 대시보드 가상 배터리 레벨을 TinyML 입력으로 사용     │
 * │        BATTERY     → BATTERY_ADC_PIN 실측 전압을 0~100%로 변환하여 사용  │
 * │                                                                         │
 * │ 3. 동적 임계값 매핑 및 확장 JSON 텔레메트리                                │
 * │    - 하드코딩 임계값 → Mutex 보호 g_config 런타임 값으로 완전 대체         │
 * │    - PUBLISH 페이로드: temp, hum, gas(저항값), battery(전원 모드 분기값) │
 * │                                                                         │
 * │ 4. QoS 0 / 1 / 2 고충실도 핸드셰이크 루프 (100% 보존)                    │
 * │    - QoS 0 : 단발 무확인 전송                                             │
 ...
 *   - knolleary/PubSubClient  @ ^2.8   : MQTT 브로커 통신
...
/* ─── Board 1 다중 노드 식별자 ────────────────────────────────────────── */
// [아키텍처 요구사항 2] Board 1의 고유 클라이언트 ID
 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
// 배터리 전압 측정 ADC 핀 (ESP32-S3: ADC1_CH0 = GPIO 1)
 ...
 *
 * [아키텍처 요구사항 1] FreeRTOS Mutex(g_config_mutex)로 스레드 안전하게 보호됩니다.
 *
 * 갱신 경로:
 ...
 *
 * 사용 경로:
 *   loop() → preprocess_tinyml_inputs()  : POWER_MODE, CURRENT_BATTERY_LEVEL 참조
 *   loop() → run_complex_agent_inference(): 임계값(temp, gas, rssi) 참조
 * ═══════════════════════════════════════════════════════════════════════════ */
struct SystemConfig {
  // ── NETWORK 섹션 ─────────────────────────────────────────────────────────
  ...
  uint8_t current_battery_level;  // 대시보드에서 설정하는 가상 배터리 레벨 (0~100 %)
                                   // EXTERNAL_5V 모드에서 TinyML 배터리 입력으로 사용
};
...
// [수정] C++17 GCC에서 char[] 배열 멤버에 designated initializer(.field=) 사용 시
// "C99 designator outside aggregate initializer" 오류 발생 → 위치 기반 초기화로 교체
...
/* ═══════════════════════════════════════════════════════════════════════════
 * [섹션 D] TinyML 전처리 입력값 구조체
 *
 * preprocess_tinyml_inputs()가 반환하는 정규화된 센서 데이터 묶음.
 * run_complex_agent_inference()와 PUBLISH 페이로드 조립에 모두 사용됩니다.
 * ═══════════════════════════════════════════════════════════════════════════ */
struct TinyMLInputs {
  ...
  float  battery_pct; // TinyML 배터리 입력 (0.0~100.0 %)
...
/*
 * BME680 온도 측정 함수 (°C)
 * 시뮬레이션: 32.5°C (기본 임계값 45°C 이하이므로 정상 상태)
 ...
 * [아키텍처 요구사항 3] PUBLISH 페이로드 gas 필드 및 동적 임계값 비교에 사용됩니다.
 * 시뮬레이션: 18.5 kΩ (기본 임계값 20 kΩ 미만 → 가스 위험 상황 시뮬레이션)
 ...
 * 시뮬레이션: WiFi.RSSI()가 반환하는 실시간 값 사용 (-80 dBm 경계값 가정)
 ...
 *
 * [아키텍처 요구사항 2 — BATTERY 분기] 하드웨어 ADC를 통해 실제 배터리
 * 전압을 측정하고 0~100% 범위로 선형 변환합니다.
 *
 * 회로 전제 조건:
 ...
 *
 * [아키텍처 요구사항 1] PubSubClient가 "gingerbread/config" 토픽 메시지를
 * 수신할 때 자동 호출됩니다. ArduinoJson으로 JSON 페이로드를 파싱하고,
 * FreeRTOS Mutex를 획득하여 g_config 구조체를 스레드 안전하게 갱신합니다.
 *
 * 수신하는 JSON 페이로드 예시:
 * {
...
static void on_mqtt_message(char *topic, byte *payload, unsigned int length) {
  ...
  // [수정] ArduinoJson v7에서 StaticJsonDocument<N>은 deprecated 되었습니다.
 ...
 *
 * [아키텍처 요구사항 1, 2] PubSubClient를 MQTT 브로커에 연결하고
...
static void mqtt_connect_and_subscribe() {
  ...
  // 인자: clientID, willTopic, willQoS, willRetain, willMessage, cleanSession
...
/* ═══════════════════════════════════════════════════════════════════════════
 * [섹션 I] TinyML 입력 전처리 함수 — preprocess_tinyml_inputs()
 *
 * [아키텍처 요구사항 2] POWER_MODE에 따라 TinyML 배터리 특징(feature)의
 * 데이터 소스를 동적으로 분기합니다.
 *
 * ┌──────────────────┬────────────────────────────────────────────────────┐
...
static TinyMLInputs preprocess_tinyml_inputs() {
  ...
  if (strcmp(mode_snapshot, "BATTERY") == 0) {
    ...
    // BATTERY_ADC_PIN에 연결된 전압 분배기 회로에서 실제 배터리 전압을
    // ADC로 측정하여 0~100% 범위로 변환한 값을 TinyML 입력으로 바인딩합니다.
    ...
    // TinyML 배터리 특징 페이로드로 직접 사용합니다 (시뮬레이션 모드).
    inputs.battery_pct = (float)virt_batt_snapshot;
    Serial.printf("[전처리] 전원모드=EXTERNAL_5V → 대시보드 가상 배터리: %.0f %%\n",
                  inputs.battery_pct);
  }

  return inputs; // 모든 필드가 채워진 TinyMLInputs 반환
}

/* ═══════════════════════════════════════════════════════════════════════════
 * [섹션 J] 동적 임계값 기반 AI QoS 추론 — run_complex_agent_inference()
 *
 * [아키텍처 요구사항 3] 기존 하드코딩 임계값(30.0°C, -75 dBm 등)을
 * 완전히 제거하고, Mutex 보호된 g_config의 런타임 동적 임계값으로 교체합니다.
 *
 * ┌───────────────┬──────────────────────────────────────────────────────┐
 * │ 긴급도(urgency)│ 판정 조건                                           │
 * ├───────────────┼──────────────────────────────────────────────────────┤
 ...
 * │ 네트워크 상태     │ 판정 조건                                      │
 * ├───────────────────┼────────────────────────────────────────────────┤
 * │  1 (불안정)       │ rssi < rssi_threshold (신호 약함)              │
 * │  0 (정상)         │ rssi >= rssi_threshold                         │
 * └───────────────────┴────────────────────────────────────────────────┘
 *
 * QoS 선택 매트릭스 (기획서 §4 준수):
 *   긴급도=1 AND 네트워크=1 → QoS 2 (최고 신뢰성, 4단계 핸드셰이크)
 *   긴급도=1 XOR 네트워크=1 → QoS 1 (신뢰성 보장, 2단계 핸드셰이크)
 *   긴급도=0 AND 네트워크=0 → QoS 0 (최대 저전력, 단발 무확인 전송)
 *
 * 매개변수:
 *   inputs     - preprocess_tinyml_inputs()가 반환한 센서 데이터
...
static QoSLevel run_complex_agent_inference(const TinyMLInputs &inputs,
  ...
  // 기본값은 g_config 초기화 값과 동일하게 설정합니다.
  ...
  // ── 데이터 긴급도 판정 ───────────────────────────────────────────────
  // 기존 하드코딩 값(30.0°C, -75 dBm) 완전 제거 → 동적 임계값 사용
  ...
  // ── QoS 선택 매트릭스 적용 (기획서 §4 로직 100% 보존) ─────────────
...
/* ═══════════════════════════════════════════════════════════════════════════
 * [섹션 K] UDP 패킷 수신 대기 함수 — wait_for_packet() (기존 로직 100% 보존)
 *
 * 특정 MsgType과 msg_id가 일치하는 응답 패킷이 올 때까지
 * 최대 2.0초 동안 폴링합니다 (기획서 §3 타임아웃 명세 준수).
 *
 * 매개변수:
...
void setup() {
  ...
  // [아키텍처 요구사항 2] Board 1 클라이언트 ID를 BOARD1_CLIENT_ID 상수로 명시합니다.
 ...
 *   4. PUBLISH 패킷 조립 (확장 JSON: temp, hum, gas, battery)
...
void loop() {
  ...
  // │   BATTERY     → BATTERY_ADC_PIN 실측 전압 (0~100% 변환)            │
  // └─────────────────────────────────────────────────────────────────────┘
  ...
  // ┌─────────────────────────────────────────────────────────────────────┐
  // │ 4단계: PUBLISH 패킷 조립                                             │
  // │ [아키텍처 요구사항 3] JSON 페이로드 확장:                             │
  // │   기존: {"temp":..., "hum":...}                                     │
  // │   추가: gas(가스 저항값 kΩ), battery(전원 모드 분기 배터리 레벨)     │
  // └─────────────────────────────────────────────────────────────────────┘
  ...
  pub_pkt.network_status  = net_status;        // AI 추론 결과 — 게이트웨이에서 모니터링 활용
  pub_pkt.data_urgency    = data_urgency;      // AI 추론 결과 — 게이트웨이에서 알람 발동 활용

  // JSON 페이로드 포맷 (확장 버전):
  //   "temp"    : BME680 온도 (°C, 소수 2자리)
  //   "hum"     : BME680 습도 (%, 소수 2자리)
  //   "gas"     : BME680 가스 저항값 (kΩ, 소수 2자리) ← [신규 추가]
  //   "battery" : 전원 모드 분기 배터리 레벨 (%, 정수) ← [신규 추가]
  snprintf(pub_pkt.payload, sizeof(pub_pkt.payload),
           ...
           ml_inputs.gas_kohm,    // BME680 가스 저항 (kΩ) [신규]
           ml_inputs.battery_pct); // 전원 모드 분기 배터리 레벨 [신규]
  ...
  if (selected_qos == QoSLevel::QoS0) {
    ...
    while (retry_count <= max_retries) {
      ...
      // PUBACK 수신 대기 (최대 2.0초, 기획서 §3 타임아웃 준수)
